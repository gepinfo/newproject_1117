...
export class productlistController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/productlist/:id', this.GpDelete);
this.router.get('/productlist/get/search', this.GpSearch);
this.router.put('/productlist', this.GpUpdate);
this.router.get('/productlist/:id', this.GpGetEntityById);
this.router.get('/productlist', this.GpGetAllValues);
this.router.post('/productlist', this.GpCreate);
this.router.get('/productlist/userid/created_by', this.GpGetNounCreatedBy);
        //#@gepdelimeterone@#
        //#@ssofacebookapiroute@#
        //#@ssogithubapiroute@#
        //#@gepbankingapiroute@#
...
public GpGetEntityById(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into productlistController.ts: GpGetEntityById');
        new ApiAdapter().get(Constant.PRODUCTSURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from productlistController.ts: GpGetEntityById');
        }).catch(err => {
            res.send(err);
        });
    }
...
public GpGetNounCreatedBy(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into productlistController.ts: GpGetNounCreatedBy');
        new ApiAdapter().get(Constant.PRODUCTSURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
            switch(req.baseUrl) { case '/mobile': res.send(result); break; case '/web': res.send(result); break; default: res.send(null); }
            new CustomLogger().showLogger('info', 'Exit from productlistController.ts: GpGetNounCreatedBy');
        }).catch(err => {
            res.send(err);
        });
    }
